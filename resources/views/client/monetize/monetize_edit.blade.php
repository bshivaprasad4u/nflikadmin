...
<section class="content">
    <div class="card card-info">
        ...
        <form method="post" class="form-horizontal" action="{{route('client.contents.monetize_update', $content->id)}}" enctype="multipart/form-data">
            <div class="card-body">
                @method('POST')
                @csrf
                <input type="hidden" name="content_id" value="{{$content->content_id}}">
                <div class="form-group row required">
                    <label class="col-form-label col-sm-4" for="">Make it Free</label>
                    <div class="col-sm-8">
                        <label><input type="radio" name="makeitfree" value="yes"> Yes</label>
                        <label><input type="radio" name="makeitfree" value="no" checked> No</label>
                    </div>
                </div>
                <div id="monetize_div">
                    <div class="form-group row required">
                        ...
                        <div class="col-sm-8">
                            <input class="form-control" name="price" placeholder="Enter Price" type="text" value="{{ old('price')??$content->price }}">
                            @error('price')
                    ...
                    <div class="form-group row required">
                        ...
                        <div class="col-sm-8">
                            <select class="form-control" name="currency">
                                ...
                                <option value="{{$currency}}" {{ ($content->currency == $currency) ? 'selected' : ''}}>
                                    {{ ucfirst($currency) }}
                                </option>
                                @endforeach
                            </select>
...
<script>
    $(function() {
        ...
        $("input[name$='makeitfree']").click(function() {
            if ($(this).val() == 'yes')
                $('#monetize_div').hide();
            else
                $('#monetize_div').show();
